...
def _is_ai_conflict_rule(rule):
    ...
    return hit and parts[-1] != ai_manual_group
...
clean_rules = [
    ...
    and rule not in BULK_DOWNLOAD_DIRECT_RULES
...
front_rules = AI_RULES + BULK_DOWNLOAD_DIRECT_RULES
...
if match_index is None:
    clean_rules = front_rules + clean_rules + [CHINAMAX_RULE] + MAINLAND_DIRECT_RULES
else:
    clean_rules = (
        front_rules
