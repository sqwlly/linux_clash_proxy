...
from .cli_render import _display_width, _pad_right, _section_heading
...
def select_one(
    ...
    *,
    ...
    def draw(position: int) -> None:
        nonlocal drawn
        drawn = _redraw(choices, position, title, drawn)
...
def _redraw(choices: list[str], index: int, title: str, drawn: int) -> int:
    ...
    for offset in range(start, end):
        ...
        padded = _pad_right(f"{marker}{choices[offset]}", label_width + 2)
