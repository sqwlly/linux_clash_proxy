...
- `status` 是否显示 `运行配置    已就绪`
- `status` 是否显示 `状态        运行中`
- `status` 是否按 `摘要 / 资源 / 流量 / 路径` 区块展示（开启图标时带 `▸` 前缀）
- `status` 是否显示 `AI 路由` 与 `当前出口`
- `status` 的 `资源` 区块是否给出 `连接数 / 运行时间 / 内存 / 日志`
- `status` 的 `流量 (今日)` 区块「按进程」是否按链路拆出 `代理↓ / 代理↑ / 直连↓ / 直连↑` 四列
