...
`status` 的流量区块带统计窗口限定词，实际渲染为 `流量 (今日)`。

`status` 的 `流量` 区块除今日总量/代理/直连外，还给出「按进程」明细：

- 默认显示前 5 个进程，`--top N` 调整，`--no-process` 关闭
- 每行包含占总流量比例，以及按链路拆分的 `代理↓ / 代理↑ / 直连↓ / 直连↑` 四个字节列
  （`直连 = 总量 − 代理`）。用字节而非单个代理占比，是为了让 `0 B` 与 `324 MB`
  这种量级差异直接可见——占比 0.1% 的进程在百分比下几乎看不出量
- 进程路径按需压缩（`$HOME` → `~`，超长时跳过 `node_modules`/`bin`/版本号等
  噪声段、保留辨识尾段并前缀 `…/`）

`status` 的 `资源` 区块展示 `连接数`/`运行时间`/`内存`/`日志`，
其中后三项来自本机 `/proc` 只读采集，取不到时该行自动省略（不影响其余输出）。
`路径` 区块在运行实例加载的 runtime 内容已落后于磁盘当前版本时，多显示一行
`配置时效    运行实例未跟随最近一次 render`（比对启动时记录的内容指纹，
路径恒定故不能靠路径差异判断）。
